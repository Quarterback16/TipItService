...
namespace TipItService.TippingStrategies
{
    public class NibbleTipster : BaseTipster, ITipster
    {
            ...
            string leagueCode)
        {
            ...
            {
                sb.AppendLine(
                    $@"{StringUtils.StringOfSize(4, pair.Key)} {pair.Value} {CurrentForm(
                            leagueCode,
                            pair.Key)}");
            }
            return sb.ToString();
    ...
    public class NibbleRating
    {
        ...
        {
            return $@"Off:{StringUtils.PadLeft(4, Offence.ToString())} Def:{StringUtils.PadLeft(4, Defence.ToString())} Tot:{StringUtils.PadLeft(4, Total().ToString())}";
        }
